/**
 * @file route.ts
 * @description Next.js route handler proxying Drive upload completion to the backend.
 * @architecture Authenticates the session, mints a service JWT, and forwards POST to the Fastify upload-complete endpoint.
 */
...
/**
 * @desc    Proxy POST /api/resources/:resourceId/upload/complete to the backend to finalize a Drive upload
 * @param   {NextRequest} req - The incoming request body
 * @returns {Promise<NextResponse>} The completed resource or an error response
 */
export async function POST(
  ...
  const res = await fetch(
    `${API_URL}/api/resources/${resourceId}/upload/complete`,
    {
      method: "POST",
      headers,
      body,
