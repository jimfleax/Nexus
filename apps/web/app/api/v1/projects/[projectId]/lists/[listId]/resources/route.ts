/**
 * @file route.ts
 * @description Next.js route handler proxying resource creation and listing to the backend.
 * @architecture Authenticates the session, mints a service JWT, and forwards POST/GET to the Fastify resources API.
 */
...
/**
 * @desc    Proxy POST /api/resources to the backend to create a resource
 * @param   {NextRequest} req - The incoming request body
 * @returns {Promise<NextResponse>} The created resource or an error response
 */
...
/**
 * @desc    Proxy GET /api/projects/:projectId/resources?listId=:listId to the backend
 * @param   {NextRequest} req - The incoming request
 * @returns {Promise<NextResponse>} The resource list or an error response
 */
  ...
  try {
    try {
      const res = await fetch(
        `${API_URL}/api/projects/${projectId}/resources?listId=${listId}`,
        {
          headers,
        },
      );
