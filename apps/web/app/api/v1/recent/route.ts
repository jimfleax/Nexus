/**
 * @file route.ts
 * @description Next.js route handler proxying recently-opened resources to the backend.
 * @architecture Authenticates the session, mints a service JWT, and forwards GET to the Fastify recent API.
 */
...
/**
 * @desc    Proxy GET /api/recent to the backend, forwarding the query string
 * @param   {NextRequest} req - The incoming request
 * @returns {Promise<NextResponse>} The recent resources or an error response
 */
export async function GET(req: NextRequest) {
  ...
  try {
    try {
      const res = await fetch(`${API_URL}/api/recent${req.nextUrl.search}`, {
        method: "GET",
        headers,
      });
      if (!res.ok) {
