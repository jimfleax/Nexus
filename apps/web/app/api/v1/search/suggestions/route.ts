/**
 * @file route.ts
 * @description Next.js route handler proxying search suggestions to the backend.
 * @architecture Authenticates the session, mints a service JWT, and forwards GET to the Fastify search-suggestions API.
 */
...
/**
 * @desc    Proxy GET /api/search/suggestions to the backend, forwarding the query string
 * @param   {NextRequest} req - The incoming request
 * @returns {Promise<NextResponse>} The suggestion list or an error response
 */
  ...
  try {
    try {
      const res = await fetch(
        `${API_URL}/api/search/suggestions${req.nextUrl.search}`,
        {
          method: "GET",
          headers,
        },
