/**
 * @file route.ts
 * @description Next.js route handler proxying full-text search to the backend.
 * @architecture Authenticates the session, mints a service JWT, and forwards GET to the Fastify search API.
 */
...
/**
 * @desc    Proxy GET /api/search to the backend, forwarding the query string
 * @param   {NextRequest} req - The incoming request
 * @returns {Promise<NextResponse>} The search results or an error response
 */
export async function GET(req: NextRequest) {
  ...
  try {
    try {
      const res = await fetch(`${API_URL}/api/search${req.nextUrl.search}`, {
        method: "GET",
        headers,
      });
      if (!res.ok) {
