/**
 * @file route.ts
 * @description Next.js route handler proxying favorite listing to the backend.
 * @architecture Authenticates the session, mints a service JWT, and forwards GET to the Fastify favorites API.
 */
...
/**
 * @desc    Proxy GET /api/favorites to the backend, forwarding the query string
 * @param   {NextRequest} req - The incoming request
 * @returns {Promise<NextResponse>} The favorite resources or an error response
 */
export async function GET(req: NextRequest) {
  ...
  try {
    try {
      const res = await fetch(`${API_URL}/api/favorites${req.nextUrl.search}`, {
        method: "GET",
        headers,
      });
      if (!res.ok) {
